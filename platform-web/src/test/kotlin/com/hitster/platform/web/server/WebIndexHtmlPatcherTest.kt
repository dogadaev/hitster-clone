...
class WebIndexHtmlPatcherTest {
    ...
    fun patchInjectsMobileViewportTouchHandlingCanvasSizingAndWakeLockSupport() {
        ...
        assertContains(patchedHtml, "wakeFallbackVideo.disableRemotePlayback = true;")
        ...
        assertContains(patchedHtml, "function currentWakeSourceType() {")
        ...
        assertContains(patchedHtml, "if (wakeFallbackVideo.duration <= 1 || currentWakeSourceType() === \"webm\") {")
        assertContains(patchedHtml, "wakeFallbackVideo.currentTime > 0.5")
        assertContains(patchedHtml, "wakeFallbackVideo.currentTime = Math.random();")
        assertContains(patchedHtml, "function resumeWakeFallbackPlayback(reason) {")
        assertContains(patchedHtml, "[\"pause\", \"ended\", \"abort\"].forEach(function(type) {")
        assertContains(patchedHtml, "resumeWakeFallbackPlayback(type);")
