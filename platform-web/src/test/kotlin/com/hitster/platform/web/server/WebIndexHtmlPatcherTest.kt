...
class WebIndexHtmlPatcherTest {
    ...
    fun patchInjectsMobileViewportTouchHandlingCanvasSizingWakeLockAndFullscreenSupport() {
        ...
        assertContains(patchedHtml, "document.addEventListener(type, handleWakeGesture, { passive: false, capture: true });")
        ...
        assertContains(patchedHtml, "wakeFallbackVideo.setAttribute(\"autoplay\", \"\")")
        assertContains(patchedHtml, "wakeFallbackVideo.setAttribute(\"loop\", \"\")")
        assertContains(patchedHtml, "wakeFallbackVideo.playsInline = true;")
        assertContains(patchedHtml, "wakeFallbackVideo.autoplay = true;")
        assertContains(patchedHtml, "wakeFallbackVideo.loop = true;")
        assertContains(patchedHtml, "wakeFallbackVideo.style.position = \"fixed\";")
        assertContains(patchedHtml, "appendSource(wakeFallbackVideo, \"webm\",")
        assertContains(patchedHtml, "appendSource(wakeFallbackVideo, \"mp4\",")
        assertContains(patchedHtml, "[\"playing\", \"pause\", \"waiting\", \"stalled\", \"suspend\", \"abort\", \"ended\"].forEach(function(type) {")
        assertContains(patchedHtml, "wakeState.media = type;")
        assertContains(patchedHtml, "document.body.appendChild(wakeFallbackVideo);")
