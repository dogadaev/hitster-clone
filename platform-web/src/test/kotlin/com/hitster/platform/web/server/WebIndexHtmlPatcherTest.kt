...
class WebIndexHtmlPatcherTest {
    ...
    fun patchInjectsMobileViewportTouchHandlingCanvasSizingAndWakeLockSupport() {
        ...
        assertContains(patchedHtml, "if (currentWakeSourceType() === \"webm\") {")
        assertContains(patchedHtml, "var resetTargetTime = 0.01;")
        assertContains(patchedHtml, "var resetThreshold = Math.max(0.5, wakeFallbackVideo.duration - 0.1);")
        assertContains(patchedHtml, "wakeFallbackVideo.currentTime >= resetThreshold")
        assertContains(patchedHtml, "wakeFallbackVideo.currentTime = resetTargetTime;")
